...
- 关键机制/公式（真实形式 + 直觉,均从 PDF 抄准）：
  - **Historical Variance Score(Eq.1)**:对样本 \(i\) 在全集 RLVR 的逐 epoch 训练准确率 \(L_i=[s_{i,1},\dots,s_{i,E}]\),取方差 \(\displaystyle v_i := \mathrm{var}(s_{i,1},\dots,s_{i,E}).\) 直觉:某样本训练准确率忽高忽低(方差大),说明它"刚好卡在模型会与不会的边界",反复给出有用的对错信号。
  - **排名(Eq.2)**:定义排列 \(\pi:[N]\to[N]\) 使 \(v_{\pi(1)}\ge\cdots\ge v_{\pi(N)}\),记 \(\displaystyle \pi_j := \pi(j)=\operatorname*{arg\,sort}_{j}\{v_l: l\in[N]\}.\) 作者反复强调这**不是关键**——关键是"几乎任何不太难也没标错的题"都能点火。
