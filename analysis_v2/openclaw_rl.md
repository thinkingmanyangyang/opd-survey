...
- 关键机制/公式（真实形式 + 直觉,从 PDF 抄准）:
  ...
  - **hint 选择**:\(\displaystyle h^\star(i)=\begin{cases}\arg\max_h \sum_i O[h,i], & \text{sequence-level}\\[2pt]\arg\max_h O[h,i], & \text{token-level}\end{cases}\) 序列级每条轨迹选一个 hint、token 级每位置选不同 hint(因 OPD loss 本身 token 级,Appendix C);二者性能相近,序列级在 general agentic 更稳。直觉:重叠高 ⇒ 师生在词表 support 上已一致 ⇒ 蒸馏在学生高密度区内移动而非推向陌生 token。
