...
- **核心散度的真实形式 + 直觉**：
  - 序列级 KLD 可精确分解为 token 级(Eq.1–3):\(\displaystyle D_{\mathrm{KL}}(p,q_\theta)=\mathbb{E}_x\,\mathbb{E}_{y\sim p(\cdot\mid x)}\Big[\log\tfrac{p(y\mid x)}{q_\theta(y\mid x)}\Big]\approx\tfrac{1}{|\mathcal{D}|}\sum_{(x,y)\in\mathcal{D}}\sum_{t}\sum_{y_t\in V}p(y_t\mid y_{<t},x)\log\tfrac{p(y_t\mid y_{<t},x)}{q_\theta(y_t\mid y_{<t},x)}.\)
