...
- **核心损失的真实形式 + 直觉**：
  ...
  - **CALD(本文核心,Eq.5)**:\(\displaystyle \mathcal{L}_{\mathrm{CALD}}=\frac{1}{2|\mathcal{D}|}\sum_{(x,y_t,y_s)\sim\mathcal{D}}\Big[D^{(\alpha)}_{\mathrm{SKL}}(x,y_t)+D^{(\alpha)}_{\mathrm{SRKL}}(x,y_s)\Big].\)即对**教师响应 \(y_t\)**(多数 \(p(y_t)\gg0\))用 SKL(抬),对**学生响应 \(y_s\)**(多数 \(p(y_s)\approx0\))用 SRKL(压)。
